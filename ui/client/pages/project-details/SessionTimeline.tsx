    ...
    <section className="mt-6" aria-label="Session timeline">
      ...
      <div className="rounded-lg border border-border/80 bg-card shadow-soft overflow-hidden">
        ...
        {dayRows.map(([date, daySessions]) => {
          ...
          return (
            <div key={date} className="flex items-center px-3 py-1 border-b border-border/10 last:border-b-0 hover:bg-secondary/10">
                  ...
                  return (
                    <Tooltip key={session.id}>
                      ...
                      <TooltipContent side="top" className="text-xs px-2.5 py-1.5">
                        ...
                        {session.note && (
                          <p className="text-muted-foreground/70 mt-0.5 max-w-[200px] truncate">{session.note}</p>
                        )}
                      </TooltipContent>
                    </Tooltip>
                  );
