...
export default function MonthlyRetrospective() {
	...
	const [year, month] = yearMonth
		? [parseInt(yearMonth.split("-")[0], 10), parseInt(yearMonth.split("-")[1], 10) - 1]
	...
	const stats = useMemo(() => {
		...
		const topProject = topProjectEntry
			? {
					id: topProjectEntry[0],
					minutes: topProjectEntry[1],
					...projectMap.get(topProjectEntry[0]),
				}
			: null;
	...
	const handleCopy = () => {
		...
		const lines = [
			...
			stats.topProject
				? `Top project: ${stats.topProject.name} (${formatDuration(stats.topProject.minutes)})`
				: "",
			stats.busiestDay
	...
	return (
		<div className="max-w-3xl mx-auto px-6 py-6 space-y-5">
			...
			{isLoading ? (
				...
				<EmptyState
					variant="chart"
					message={`No sessions recorded in ${MONTH_NAMES[month]} ${year}`}
				/>
			) : (
				<>
					{/* Stat cards */}
