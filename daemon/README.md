...
| `beatsd doctor` | Diagnostics — checks pairing, API reachability, editor-listener port, Accessibility permission, and (informational) flow-data flow ("N windows · avg X (last hour)"). The flow check never fails the command since a freshly-paired daemon has zero windows; it just surfaces the count so you can spot a daemon that's paired but not collecting. Exit 1 on any other failure, suitable for `beatsd doctor && beatsd run` startup scripts. |
| `beatsd status` | Snapshot of right-now state — pair, daemon-running probe, current timer state from the API, and a one-line flow summary for the last hour ("N windows · avg X · peak Y") so you can confirm at a glance the flow pipeline is producing results. |
| `beatsd recent [--minutes N] [--repo P] [--language L] [--bundle B] [--json]` | Print the last hour (default) of flow windows in a small terminal-friendly table — time, score, dominant app, repo. Optional filters mirror the web Insights chips: `--repo` matches the editor workspace path, `--language` matches a VS Code language id, `--bundle` matches a macOS bundle id. Filters AND-compose. `--json` emits the raw windows as a JSON array instead of the table, intended for shell scripting (`beatsd recent --language go --json \| jq '.[] \| select(.flow_score > 0.7)'`). Useful for "what was the daemon seeing the last hour, on this repo / in Go / inside Xcode?" without opening the web UI. |
| `beatsd top [--minutes N] [--json]` | Print top-5 leaderboards by repo / language / app for the recent window — terminal mirror of the FlowByRepo / FlowByLanguage / FlowByApp cards on the web Insights page. Each row shows the bucket label, average flow score (out of 100), and tracked minutes (count of 1-minute windows). Sorted by minutes, ties broken by avg score so higher-quality buckets surface first. `--json` emits the three leaderboards as a single object (`{by_repo: [...], by_language: [...], by_app: [...]}`) for piping into jq — JSON output uses raw bundle ids on the by_app axis instead of the human "coding" / "browser" category labels the table uses. |
| `beatsd stats [--minutes N] [--repo P] [--language L] [--bundle B] [--json]` | Print a one-line headline summary: count · avg · peak (with HH:MM) · best repo. Hits the API's `/flow-windows/summary` endpoint so it costs one round-trip — designed for shell prompts and status bars. `--json` emits the raw FlowWindowSummary object instead of the line. |
| `beatsd open [--repo PATH] [--language ID] [--bundle ID] [--print]` | Open the Beats web UI Insights page in the system browser. The three filter flags AND-compose into the same `?repo=…&language=…&bundle=…` URL scheme the page uses for click-to-filter chip persistence (so a deep-link from the CLI lands on the exact view a chip click would). With `--print` writes the URL to stdout instead of launching, useful for shell pipelines (`beatsd open --repo $(pwd) --print \| pbcopy`) and headless setups. Mirrors the VS Code extension's "Beats: Open Insights" command. Web URL is configurable via `[ui] base_url` in `daemon.toml`; defaults to `http://localhost:8080`. |
...
| `beatsd config` | Print the loaded daemon configuration: API + UI URLs, collector intervals, and the path to `daemon.toml`. The device token is intentionally NOT printed — output is meant to be pasteable into bug reports without leaking credentials. |
