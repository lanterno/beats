...
| `beatsd open [--repo PATH] [--print]` | Open the Beats web UI Insights page in the system browser. With `--repo` deep-links to a filtered view (`?repo=…`) — same URL scheme the page uses for click-to-filter chip persistence. With `--print` writes the URL to stdout instead of launching, useful for shell pipelines (`beatsd open --repo $(pwd) --print \| pbcopy`) and headless setups. Mirrors the VS Code extension's "Beats: Open Insights" command. Web URL is configurable via `[ui] base_url` in `daemon.toml`; defaults to `http://localhost:8080`. |
