...
| `beatsd open [--repo PATH]` | Open the Beats web UI Insights page in the system browser. With `--repo` deep-links to a filtered view (`?repo=…`) — same URL scheme the page uses for click-to-filter chip persistence. Mirrors the VS Code extension's "Beats: Open Insights" command for users who'd rather stay in the shell. Web URL is configurable via `[ui] base_url` in `daemon.toml`; defaults to `http://localhost:8080`. |
